...
export function WithdrawalCenter({ activeSection, onNavigate }: { activeSection: ActiveSection; onNavigate: (section: Exclude<ActiveSection, 'Retiros'>) => void }) {
  ...
  return <div className="withdrawal-center">
    <div className="relative mt-2 w-full"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><label className="relative block"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true"><svg viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current stroke-[2.4]"><circle cx="10.8" cy="10.8" r="6.5" /><path d="m16 16 4.2 4.2" /></svg></span><input aria-label="Buscar número de tarjeta o cliente" type="search" value={groupSearch} placeholder="# de tarjeta o nombre de cliente" onChange={event => setGroupSearch(event.currentTarget.value)} className="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-bold text-slate-800 outline-none focus:border-emerald-500" /></label></div><label className="shrink-0 cursor-pointer rounded-2xl bg-emerald-700 px-3 py-3 text-center text-xs font-black text-white shadow-sm hover:bg-emerald-800">{importing ? 'Importando...' : 'Importar Excel'}<input className="hidden" type="file" accept=".xlsx,.xls" disabled={importing} onChange={event => { const file = event.target.files?.[0]; if (file) importWorkbook(file) }} /></label></div>{personResults.length > 0 && <div className="absolute z-30 mt-2 max-h-72 w-full overflow-y-auto rounded-2xl border border-emerald-200 bg-white shadow-xl shadow-emerald-950/10">{personResults.slice(0, 5).map(result => <button key={`${result.group.id}-${result.member.sourceRow}`} onClick={() => { setOpenGroupId(result.group.id); setReadingMember({ groupId: result.group.id, sourceRow: result.member.sourceRow }); setSelectedMember(null); setSelectionMode(false); setGroupSearch('') }} className="flex w-full items-center justify-between gap-3 border-b border-emerald-50 px-3 py-2.5 text-left last:border-0 hover:bg-emerald-50"><span className="min-w-0"><b className="block text-[13px] leading-5 text-slate-900">{result.member.fullName}</b><small className="block text-[11px] font-bold leading-4 text-slate-500">N° tjta: {result.member.cardNumber} · {result.group.organizationName} · Grupo {result.group.groupNumber}</small></span><span className="shrink-0 rounded-lg bg-emerald-100 px-2.5 py-1.5 text-xs font-black text-emerald-800">Ver</span></button>)}</div>}{groupSearch.trim() && personResults.length === 0 && !filteredGroups.length && <p className="absolute z-30 mt-2 w-full rounded-xl bg-white px-3 py-2 text-sm font-bold text-slate-500 shadow-lg">No se encontró un grupo, tarjeta o cliente con ese dato.</p>}</div>
