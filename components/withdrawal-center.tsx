...
export function WithdrawalCenter({ activeSection, onNavigate }: { activeSection: ActiveSection; onNavigate: (section: Exclude<ActiveSection, 'Retiros'>) => void }) {
  ...
  return <div className="withdrawal-center">
    <div className="mt-2 max-w-lg"><label className="relative block"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true"><svg viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current stroke-[2.4]"><circle cx="10.8" cy="10.8" r="6.5" /><path d="m16 16 4.2 4.2" /></svg></span><input aria-label="Buscar grupo, número de tarjeta o cliente" type="search" value={groupSearch} placeholder="# de grupo, N° tjta o cliente" onChange={event => setGroupSearch(event.currentTarget.value)} className="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-bold text-slate-800 outline-none focus:border-emerald-500" /></label>{personResults.length > 0 && <div className="mt-2 overflow-hidden rounded-2xl border border-emerald-200 bg-white shadow-sm">{personResults.slice(0, 6).map(result => <button key={`${result.group.id}-${result.member.sourceRow}`} onClick={() => { setOpenGroupId(result.group.id); setReadingMember({ groupId: result.group.id, sourceRow: result.member.sourceRow }); setSelectedMember(null); setSelectionMode(false) }} className="flex w-full items-center justify-between gap-3 border-b border-emerald-50 px-4 py-3 text-left last:border-0 hover:bg-emerald-50"><span><b className="block text-sm text-slate-900">{result.member.fullName}</b><small className="mt-0.5 block text-xs font-bold text-slate-500">N° tjta: {result.member.cardNumber} · {result.group.organizationName} · Grupo {result.group.groupNumber}</small></span><span className="rounded-lg bg-emerald-100 px-2 py-1 text-xs font-black text-emerald-800">Ver</span></button>)}</div>}{groupSearch.trim() && personResults.length === 0 && !filteredGroups.length && <p className="mt-2 text-sm font-bold text-slate-500">No se encontró un grupo, tarjeta o cliente con ese dato.</p>}</div>
