...
export function WithdrawalCenter({ activeSection, onNavigate }: { activeSection: ActiveSection; onNavigate: (section: Exclude<ActiveSection, 'Retiros'>) => void }) {
  ...
  const personResults = useMemo<PersonSearchResult[]>(() => searchTerm ? groups.flatMap(group => (membersByGroup[group.id] ?? []).filter(member => searchByGroupNumber ? member.cardNumber.includes(groupSearch.trim()) : normalizedSearch(member.fullName).includes(searchTerm)).map(member => ({ group, member }))) : [], [groups, membersByGroup, groupSearch, searchByGroupNumber, searchTerm])
  const filteredGroups = useMemo(() => searchTerm ? groups.filter(group => (searchByGroupNumber && group.groupNumber.includes(groupSearch.trim())) || personResults.some(result => result.group.id === group.id)) : groups, [groups, groupSearch, personResults, searchByGroupNumber, searchTerm])
  ...
  return <div className="withdrawal-center">
    <div className="relative mt-2 w-full"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><label className="relative block"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true"><svg viewBox="0 0 24 24" className="h-5 w-5 fill-none stroke-current stroke-[2.4]"><circle cx="10.8" cy="10.8" r="6.5" /><path d="m16 16 4.2 4.2" /></svg></span><input aria-label="Buscar número de tarjeta o cliente" type="search" value={groupSearch} placeholder="# de tarjeta o nombre de cliente" onFocus={() => setSearchFocused(true)} onBlur={() => window.setTimeout(() => setSearchFocused(false), 120)} onChange={event => setGroupSearch(event.currentTarget.value)} className="w-full rounded-2xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm font-bold text-slate-800 outline-none focus:border-emerald-500" /></label></div><label className="shrink-0 cursor-pointer rounded-2xl bg-emerald-700 px-3 py-3 text-center text-xs font-black text-white shadow-sm hover:bg-emerald-800">{importing ? 'Importando...' : 'Importar Excel'}<input className="hidden" type="file" accept=".xlsx,.xls" disabled={importing} onChange={event => { const file = event.target.files?.[0]; if (file) importWorkbook(file) }} /></label></div>{searchFocused && personResults.length > 0 && <div className="absolute z-30 mt-2 max-h-72 w-full overflow-y-auto rounded-2xl border border-emerald-200 bg-white shadow-xl shadow-emerald-950/10">{personResults.slice(0, 5).map(result => <button key={`${result.group.id}-${result.member.sourceRow}`} onClick={() => { setOpenGroupId(result.group.id); setReadingMember({ groupId: result.group.id, sourceRow: result.member.sourceRow }); setSelectedMember(null); setSelectionMode(false); setGroupSearch(''); setSearchFocused(false) }} className="flex w-full items-center justify-between gap-3 border-b border-emerald-50 px-3 py-2.5 text-left last:border-0 hover:bg-emerald-50"><span className="min-w-0"><b className="block text-[13px] leading-5 text-slate-900">{result.member.fullName}</b><small className="block text-[11px] font-bold leading-4 text-slate-500">N° tjta: {result.member.cardNumber} · {result.group.organizationName} · Grupo {result.group.groupNumber}</small></span><span className="shrink-0 rounded-lg bg-emerald-100 px-2.5 py-1.5 text-xs font-black text-emerald-800">Ver</span></button>)}</div>}{searchFocused && groupSearch.trim() && personResults.length === 0 && !filteredGroups.length && <p className="absolute z-30 mt-2 w-full rounded-xl bg-white px-3 py-2 text-sm font-bold text-slate-500 shadow-lg">No se encontró un grupo, tarjeta o cliente con ese dato.</p>}</div>
    <section className="mt-2 rounded-3xl bg-white p-6 shadow-sm">
      <div className="withdrawal-intro"><h2 className="font-black">Retiros por grupos</h2><p className="mt-1 text-sm text-slate-500">Activa “Seleccionar”, pulsa una persona y registra el estado de su tarjeta.</p></div>
      {message && <p className="mt-5 rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-bold text-emerald-800">{message}</p>}

      {groups.length > 0 && <div className="withdrawal-groups-panel mt-7"><p className="newgen-registered-title text-sm font-black text-slate-800">Grupos registrados</p><div className="withdrawal-groups-layout mt-3">
        <div className="newgen-groups-list flex gap-2 overflow-x-auto pb-2 lg:block lg:max-h-[640px] lg:space-y-2 lg:overflow-y-auto lg:overflow-x-hidden lg:pr-1">{filteredGroups.map(group => <button key={group.id} onClick={() => { setOpenGroupId(current => current === group.id ? null : group.id); setSelectedMember(null); setSelectionMode(false) }} className={openGroupId === group.id ? 'newgen-group-card w-52 shrink-0 rounded-2xl bg-emerald-700 p-4 text-left text-white shadow-lg shadow-emerald-900/15 lg:w-full' : 'newgen-group-card w-52 shrink-0 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-left text-slate-800 hover:border-emerald-300 lg:w-full'}><p className={openGroupId === group.id ? 'withdrawal-group-period text-[10px] font-black uppercase tracking-[.12em] text-emerald-100' : 'withdrawal-group-period text-[10px] font-black uppercase tracking-[.12em] text-slate-500'}>{group.periodLabel}</p><p className="withdrawal-group-organization mt-1 font-black">{group.organizationName}</p><p className={openGroupId === group.id ? 'withdrawal-group-number mt-1 text-sm font-bold text-emerald-100' : 'withdrawal-group-number mt-1 text-sm font-bold text-emerald-700'}>Grupo {group.groupNumber}</p><p className="newgen-mobile-group-label">GRUPO {group.groupNumber} <span>({group.organizationName})</span></p></button>)}{filteredGroups.length === 0 && <p className="w-full rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">No se encontró ese grupo.</p>}</div>
        {selectedGroup && <div className="withdrawal-group-modal-backdrop" role="dialog" aria-modal="true" aria-label={`Grupo ${selectedGroup.groupNumber}`} onMouseDown={event => { if (event.target === event.currentTarget) { setOpenGroupId(null); setSelectedMember(null); setSelectionMode(false) } }}><div className="withdrawal-detail withdrawal-group-modal rounded-3xl border border-emerald-400/30 bg-emerald-950 p-3 text-white shadow-2xl sm:p-4"><><div className="withdrawal-group-modal-header flex flex-col gap-2 border-b border-emerald-300/20 pb-3 sm:flex-row sm:items-end sm:justify-between"><div className="withdrawal-group-modal-title"><p>GRUPO {selectedGroup.groupNumber} <span>({selectedGroup.organizationName})</span></p></div><div className="withdrawal-group-modal-controls flex items-end justify-between gap-3 sm:justify-end"><p className="text-sm font-bold text-emerald-100/75">{selectedGroup.periodLabel}</p><div className="flex flex-col items-end gap-2"><button onClick={() => { setOpenGroupId(null); setSelectedMember(null); setSelectionMode(false) }} className="grid h-9 w-9 place-items-center rounded-full bg-white/10 text-xl font-bold text-white hover:bg-white/20" aria-label="Cerrar grupo">×</button>{readingMemberInfo && (readingMemberInfo.bankName || readingMemberInfo.accountNumber || readingMemberInfo.accountHolder) && <button onClick={() => setShowAccountInfo(true)} className="rounded-xl border border-emerald-300/30 bg-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-wide text-emerald-50 hover:bg-white/20">Info de cuenta</button>}</div></div></div>
          ...
          {loadingGroup ? <p className="py-8 text-sm font-bold text-slate-500">Cargando integrantes...</p> : <>
            <div className="mt-4 space-y-2 md:hidden">
              {groupMembers.map((member, index) => {
                ...
                return <button type="button" key={`${member.fullName}-${index}`} onClick={() => interactWithMember(member, action)} onDoubleClick={() => { if (!action) setSelectedMember(member) }} className={`w-full rounded-2xl border p-3 text-left shadow-sm ${tone} ${selectionMode ? 'active:scale-[.99]' : ''}`}>
                  <div className="withdrawal-member-summary flex items-start justify-between gap-2"><p className="min-w-0 text-sm font-black text-slate-900">{member.fullName}</p><span className="shrink-0 rounded-lg bg-emerald-100 px-2 py-1 text-xs font-black text-emerald-800">N° tjta {member.cardNumber}</span></div>
                  <div className="mt-2 grid grid-cols-2 gap-2 border-t border-slate-200/70 pt-2 text-left">
