  ...
  return <div className="withdrawal-center">
    ...
    <section className="mt-2 rounded-3xl bg-white p-6 shadow-sm">
      ...
      {groups.length > 0 && <div className="withdrawal-groups-panel mt-7"><p className="newgen-registered-title text-sm font-black text-slate-800">Grupos registrados</p><div className="withdrawal-groups-layout mt-3">
        ...
        {selectedGroup && <div className="withdrawal-group-modal-backdrop" role="dialog" aria-modal="true" aria-label={`Grupo ${selectedGroup.groupNumber}`} onMouseDown={event => { if (event.target === event.currentTarget) { setOpenGroupId(null); setSelectedMember(null); setSelectionMode(false) } }}><div className="withdrawal-detail withdrawal-group-modal rounded-3xl border border-emerald-400/30 bg-emerald-950 p-3 text-white shadow-2xl sm:p-4"><><div className="withdrawal-group-modal-header flex flex-col gap-2 border-b border-emerald-300/20 pb-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="withdrawal-group-title text-emerald-200">Grupo {selectedGroup.groupNumber}</p><h3 className="mt-1 text-xl font-black text-white">{selectedGroup.organizationName}</h3></div><div className="withdrawal-group-modal-controls flex items-end justify-between gap-3 sm:justify-end"><p className="text-sm font-bold text-emerald-100/75">{selectedGroup.periodLabel}</p><div className="flex flex-col items-end gap-2"><button onClick={() => { setOpenGroupId(null); setSelectedMember(null); setSelectionMode(false) }} className="grid h-9 w-9 place-items-center rounded-full bg-white/10 text-xl font-bold text-white hover:bg-white/20" aria-label="Cerrar grupo">×</button>{readingMemberInfo && (readingMemberInfo.bankName || readingMemberInfo.accountNumber || readingMemberInfo.accountHolder) && <button onClick={() => setShowAccountInfo(true)} className="rounded-xl border border-emerald-300/30 bg-white/10 px-3 py-2 text-[10px] font-black uppercase tracking-wide text-emerald-50 hover:bg-white/20">⌁ Información de cuenta</button>}<button onClick={() => { setSelectionMode(current => !current); setSelectedMember(null) }} className={selectionMode ? 'rounded-xl bg-white/15 px-3 py-2 text-xs font-black text-white' : 'rounded-xl bg-emerald-400 px-3 py-2 text-xs font-black text-emerald-950 hover:bg-emerald-300'}>{selectionMode ? 'Cancelar' : 'Seleccionar'}</button></div></div></div>
            ...
            <div className="mt-4 space-y-2 md:hidden">
              {groupMembers.map((member, index) => {
                ...
                return <button type="button" key={`${member.fullName}-${index}`} onClick={() => interactWithMember(member, action)} className={`w-full rounded-2xl border p-3 text-left shadow-sm ${tone} ${selectionMode ? 'active:scale-[.99]' : ''}`}>
                  <p className="text-sm font-black text-slate-900">{member.fullName} <span className="whitespace-nowrap text-emerald-800">(N° tjta {member.cardNumber})</span></p>
